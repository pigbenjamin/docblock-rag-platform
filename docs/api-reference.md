...
建立資料夾。呼叫者需要對 `parent_id` 有 `upload` 權限（部門根資料夾的 owner 部門管理員
自動具備）。根資料夾（部門根）不透過這支 API 建立：既有部署由
`scripts/migrate_fb1_nodes_acl.sql` 產生，新部門出現時由 webhook-service 的使用者同步
自動補建（含部門成員 browse/query/read entries 與 Public 對該部門的 allow entries）。
...
`subject_type` 僅接受 `user` / `department`（管理員身分不寫進節點 ACL——owner-KM 判定
查的是 `department_admins` 表；DB 裡既有的 `('role','dept:X:role:KM')` entries 仍然有效，
語意是「X 部門的管理員們」，由 authz 從 `department_admins` 合成比對）。`effect` 僅
`allow` / `deny`（detail/summary 分級已隨 D5 拿掉）。授予 `manage_acl` action = 把共同
管理權交給該 subject；只給 `browse`/`query`/`read` =單純分享，不含管理權。
...
列出 Keycloak 頂層群組，供前端下拉選單使用（透過 `user-sync-service` client 即時查詢
Keycloak admin API）。**所有頂層群組都是部門**（D10）——Keycloak 群組樹與 HR 系統連動、
可能有多層（處/課），但子單位在 v1 一律忽略（D11）。唯一例外是 `Public`：全部門可瀏覽/
查詢/讀取的共用根資料夾，不是部門，會被排除。

部門管理員**不再**從 Keycloak 群組結構推導（沒有 KM 子群組了），改由平台自建的
`department_admins` 表管理，見下方管理員端點。
...
> 純資訊性質，**不參與任何授權判斷**——授權一律查詢 `user_principal` / `department_admins` /
> `global_admins` / `acl_entries`，不會查這支端點。

---

### GET /v1/departments/{department}/admins

列出部門管理員名單（FB-6）。任何已認證使用者可讀（「要找誰申請權限」的通訊錄性質）。
`department` 用的是 `nodes.owner_department_id` 那套值。未知部門回 404。
...
```json
{
  "department": "A",
  "admins": [
    { "user_id": "xxxxxxxx-...", "created_by": "yyyyyyyy-...", "created_at": "2026-07-15T..." }
  ]
}
```
...
### PUT /v1/departments/{department}/admins

整批取代部門管理員名單。可呼叫者：**全域管理員**或**該部門現任管理員**。部門管理員
可以交棒（把自己移出、留下別人），但不能把名單清空——讓部門無人可管是全域管理員
才能做的決定（400）。寫入 `audit_logs`（`department_admins.update`）。

管理員身分就是 authz 的 owner-KM 捷徑來源：名單上的人對 owner 是該部門的所有節點
全 action 放行、deny 蓋不掉（anti-lockout），寫入後立即生效。

**Request Body**：`{ "user_ids": ["xxxxxxxx-...", ...] }`（非 UUID 回 400）

**Response**：`{ "department": "A", "user_ids": [...] }`

---

### GET /v1/global-admins

列出全域管理員。任何已認證使用者可讀。

**Response**：`{ "admins": [ { "user_id", "created_by", "created_at" } ] }`

---

### PUT /v1/global-admins

整批取代全域管理員名單。僅全域管理員可呼叫，且結果不得為空（anti-lockout，400）——
第一位全域管理員需直接 seed 進 DB：
`INSERT INTO global_admins (tenant_id, user_id) VALUES ('<tenant>', '<uuid>');`。
寫入 `audit_logs`（`global_admins.update`）。

全域管理員對**所有**節點全 action 放行（含 Public 子樹——Public 因此有人可管），
並負責指派各部門管理員。

**Request Body / Response**：同部門管理員端點（`user_ids`）。
...
接收 Keycloak 用戶事件，同步 `user_principal` 表。寫入的 principal 只有兩種：
`user`（`user:{uuid}`）與 `department`（頂層群組名，多層群組樹只取第一段——子單位
v1 忽略，D11）。**不再產生 `role` principal**（FB-6：管理員名單改由
`department_admins` 表管理，與 Keycloak 群組結構脫鉤）。

同步時會順帶補齊部門基礎設施（只增不減）：新部門的根資料夾＋部門成員
browse/query/read entries、以及 Public 根資料夾對該部門的 allow entries（D2）。
部門消失時不自動刪除任何東西——資料夾清理是破壞性操作，留給管理員手動處理。
