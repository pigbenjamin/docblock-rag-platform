...
- Ollama 正在執行並載入 `bge-m3`、`qwen3:8b`
...
| `07_search_open.py` | 開放搜尋（無 ACL 過濾） | < 20s |
...
| `12_reupload_same_docid.py` | 相同 doc_id 重複上傳：version 不遞增、document_id 不變、chunks 不重複 | 2~10 分鐘 |
...
```bash
ADMIN_API=http://staging:8765 python3 tests/01_health_check.py
...
- **03、10** 會建立臨時文件（`test-upload-xxx`、`delete-test-xxx`），不會污染現有資料
- **12** 場景 A 只需 `TEST_PDF`；場景 B（內容變更）需額外提供 `SECOND_PDF` 環境變數，否則略過
