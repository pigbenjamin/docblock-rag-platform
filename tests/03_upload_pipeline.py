...
POST /v1/documents/upload — 上傳 PDF，觸發完整 ingest pipeline
...
doc_id = f"test-upload-{uuid.uuid4().hex[:8]}"
info(f"使用 doc_id={doc_id!r}，PDF={TEST_PDF}")
...
with open(TEST_PDF, "rb") as f:
    r = requests.post(
        ...
        data={"doc_id": doc_id, "title": "Test Upload Document"},
        timeout=30,
    )
...
if job_id and body.get("status") == "submitted":
    ok(f"upload 成功 → job_id={job_id}")
else:
...
while elapsed < MAX_WAIT:
    r = requests.get(f"{ADMIN_API}/v1/documents/job/{job_id}", timeout=10)
    if r.status_code != 200:
